...
void Trace::transfer(const char* name) {
	...
	for(int i=0;i<steps;i++) {
		float f=((float)i)/((float)steps)*0.5f;
		std::complex<float> iw=std::complex<float>(0.0f,f*(float)M_PI*2.0f);
		...
		if ((fTimesPi*fTimesPi-e*e)>0) { // root ok
			...
			if (taps) {
				...
				for(int i=0;i<((taps-1)/2);i++) {
					float coeff=(1/w)*exp(-e*(float)i)*sin(w*(float)i);
